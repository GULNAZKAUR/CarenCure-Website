...
<input type="otp" placeholder="otp" class="form-control" name="otp" required="required" /></div><br /><br />
<input type="password" placeholder="new password" title="Minimum 8 charcters that should include upper case,lower case,special characters,numbers." pattern="(?=^.{8,}$)((?=.*\d)|(?=.*\W+))(?![.\n])(?=.*[A-Z])(?=.*[a-z]).*$" class="form-control" name="npwd" required="required" /><br /><br />
<input type="password" title="Minimum 8 charcters that should include upper case,lower case,special characters,numbers." pattern="(?=^.{8,}$)((?=.*\d)|(?=.*\W+))(?![.\n])(?=.*[A-Z])(?=.*[a-z]).*$" placeholder="confirm password" class="form-control" name="cpwd" required="required" /><br /><br />
...
<input type="submit" value="submit" name="save" class="btn btn-dark btn-block"  style="color:#fff" /><br />
	...
	if(isset($_POST['save']))
	{
		if($_POST['otp']==$_SESSION['code'])
		{
			...
			{
				$sql="UPDATE addexpert SET password ='$_POST[npwd]' where emailid='$_GET[id]';";
				if($conn->query($sql)===TRUE)
				echo"<script>window.alert('Password Changed..Login to Your Account');
				window.location='expert login.php'; </script>";
